...
class CreateFunctions extends Migration
{
    public function up()
    {
        // TODO: Finish fn_acornassociated_calendar_create_activity_log_event
        $this->createFunction('fn_acornassociated_calendar_create_activity_log_event', array(
        ...
        <<<SQL
            ...
            select into event_status_id id from public.acornassociated_calendar_event_statuses limit 1;
            insert into public.acornassociated_calendar_event_types(name, colour, style) values('Create', '#091386', 'color:#fff') returning id into event_type_id;
        
            insert into public.acornassociated_calendar_events(calendar_id, owner_user_id) values(calendar_id, owner_user_id) returning id into new_event_id;
...
SQL
        ...
        $this->createFunction('fn_acornassociated_calendar_seed', array(), 'void', array(),
        <<<SQL
...
SQL
        ...
        $this->createFunction('fn_acornassociated_calendar_trigger_created_at_event', [], 'trigger', array(
                ...
                table_title   := replace(replace(TG_TABLE_NAME, 'acornassociated_', ''), '_', ' ');
                ...
                select into event_type_id id from acornassociated_calendar_event_types where activity_log_related_oid = TG_RELID;
                if event_type_id is null then
                    -- TODO: Colour?
                    -- TODO: acornassociated.?::lang.models.?.label
                    insert into public.acornassociated_calendar_event_types(name, activity_log_related_oid) values(table_title, TG_RELID) returning id into event_type_id;
                end if;
                ...
                case 
                    ...
                    when TG_OP = 'UPDATE' then 
                        ...
                        if NEW.updated_at_event_id is null then
                            ...
                            update public.acornassociated_calendar_event_parts set 
                                "start"   = event_time, 
                                "end"     = event_time,
...
SQL
    ...
    {
        Schema::dropIfExists('fn_acornassociated_calendar_trigger_created_at_event');
        Schema::dropIfExists('fn_acornassociated_calendar_seed');
    }
}
